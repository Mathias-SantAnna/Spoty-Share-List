...
def all_genres():
    if request.method == "POST":
        ...
        if request.form.get("edit"):
            ...
            if not exists:
                ...
                mongo.db.music_genre.update(
                    {"_id": ObjectId(music_genre["_id"])}, music_genre)
        ...
        if not exists:
            ...
            flash(
                music_genre["genre_name"] + " Already Exists! Try another one")
...
def delete_genre(genre_id):
    ...
    mongo.db.music_genre.remove({"_id": ObjectId(genre_id)})
    flash("Genre deleted")
...
def profile(username):
    ...
    for playlist in user_playlist:
        ...
        genre = mongo.db.music_genre.find_one(
            {"_id": ObjectId(playlist['genre'])})
        playlist['genre_name'] = genre['genre_name']
